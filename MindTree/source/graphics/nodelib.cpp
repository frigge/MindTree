...
QSize NodeLibWidget::sizeHint()    const
{
    ...
    QTreeWidget(parent), NodeID(0)
{
    ...
    NodeDataBase::regCB([this]{this->update();});
}
...
{
    foreach(AbstractNodeFactory *f, NodeDataBase::getFactories()){
        int id = NodeType(f->getType()).getID();
        auto item = new QTreeWidgetItem(QStringList() << f->getLabel() << QString::number(id));
        addTopLevelItem(item);
        NodeLibItem *nli = new NodeLibItem;
        nli->item = item;
        nli->factory = f;
        nodeLibItems.insert(id, nli);
    }
}
...
void NodeLib::addGroup(QString name, QString parent)    
{
    if(!NodeGroups.keys().contains(name)) {
        QTreeWidgetItem *newgroup = new QTreeWidgetItem(QStringList() << name << QString::number(-1));
...
void NodeLib::addNode(QString name, QString group, MindTree::AbstractNodeFactory *factory)
{
    addGroup(group);
...
//        return;
    ...
    if(selectedItems().first()->childCount() > 0) return;
