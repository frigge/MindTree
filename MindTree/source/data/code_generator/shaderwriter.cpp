...
void ShaderCodeGenerator::insertLoopVar(const DNode *n, const DSocket *socket)    
{
    ...
    if(c->getOutputs()->getNodeType() ==LOOPOUTSOCKETS){
        ...
        const DoutSocket *out = c->getSocketInContainer(socket)->toOut();
...
QString ShaderCodeGenerator::writeString(const DoutSocket *socket)
{
    ...
    value.append("\"");
    value.append(stringnode->getInSockets().first()->getProperty().getData<QString>());
    value.append("\"");
...
QString ShaderCodeGenerator::writeFloat(const DoutSocket *socket)
{
    ...
    value.append(QString::number(floatnode->getInSockets().first()->getProperty().getData<float>()));
    if(floatnode->isShaderInput())
    {
        output.append("float ");
        output.append(getVariable(socket));
        addParameter(getVariable(socket), socket->getType().getType());
        //if(socket->isArray())
        //    output.append("[" + QString::number(socket->getArrayLength()) + "]");
        output.append(" = ");
        output.append(value);
